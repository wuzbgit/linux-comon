...
CROSSCOMPILE_TOOLCHAIN_LIST=(
    [imx8m]="${TOP_DIR}/toolchain/CrossCompile_imx8m.cmake"
    [t507]="${TOP_DIR}/toolchain/CrossCompile_t507.cmake"
)
...
DST_ROOTFS=/opt/Project/iScanner/iscanner-buildroot/board/boc-tech/rootfs/
function do_copy()
{
    if [ -d ${DST_ROOTFS} ] && [ -d ${OUT_ROOTFS_DIR} ];then
        echo "copy file to buildroot board"
        cp ${OUT_ROOTFS_DIR}/* ${DST_ROOTFS}/ -rf
    else
        echo "${OUT_ROOTFS_DIR} or ${DST_ROOTFS} does not exsit"
    fi 
}
